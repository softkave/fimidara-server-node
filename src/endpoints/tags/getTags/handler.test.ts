...
import getTags from './handler.js';
import {GetTagsEndpointParams} from './types.js';
...
describe('getTags', () => {
  test("workspace's tag returned", async () => {
    ...
    const reqData = RequestData.fromExpressRequest<GetTagsEndpointParams>(
      mockExpressRequestWithAgentToken(userToken),
      {workspaceId: workspace.resourceId}
    );
    const result = await getTags(reqData);
  ...
  test('pagination', async () => {
    ...
    const pageSize = 10;
    let page = 0;
    let reqData = RequestData.fromExpressRequest<GetTagsEndpointParams>(
      mockExpressRequestWithAgentToken(userToken),
      {page, pageSize, workspaceId: workspace.resourceId}
    );
    let result = await getTags(reqData);
    assertEndpointResultOk(result);
    expect(result.page).toBe(page);
    expect(result.tags).toHaveLength(calculatePageSize(count, pageSize, page));

    page = 1;
    reqData = RequestData.fromExpressRequest<GetTagsEndpointParams>(
      mockExpressRequestWithAgentToken(userToken),
      {page, pageSize, workspaceId: workspace.resourceId}
    );
    result = await getTags(reqData);
    assertEndpointResultOk(result);
    expect(result.page).toBe(page);
    expect(result.tags).toHaveLength(calculatePageSize(count, pageSize, page));
  });
