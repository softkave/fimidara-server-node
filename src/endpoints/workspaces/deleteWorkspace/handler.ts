...
const deleteWorkspace: DeleteWorkspaceEndpoint = async (context, instData) => {
  ...
  await waitOnPromises([
    ...
    context.data.collaborationRequest.deleteManyByQuery(
      EndpointReusableQueries.getByWorkspaceId(workspace.resourceId)
    ),

    // Program tokens
    context.data.programAccessToken.deleteManyByQuery(
      EndpointReusableQueries.getByWorkspaceId(workspace.resourceId)
    ),

    // Client tokens
    context.data.clientAssignedToken.deleteManyByQuery(
      EndpointReusableQueries.getByWorkspaceId(workspace.resourceId)
    ),

    // PermissionGroups
    context.data.permissiongroup.deleteManyByQuery(
      EndpointReusableQueries.getByWorkspaceId(workspace.resourceId)
    ),
    ...
    // Tags
    context.data.tag.deleteManyByQuery(
      EndpointReusableQueries.getByWorkspaceId(workspace.resourceId)
    ),

    // Assigned items
    context.data.assignedItem.deleteManyByQuery(
      EndpointReusableQueries.getByWorkspaceId(workspace.resourceId)
    ),
    ...
    context.data.workspace.deleteOneByQuery(
...
async function internalDeleteFilesByWorkspaceId(context: IBaseContext, workspaceId: string) {
  ...
  const files = await context.data.file.getManyByQuery(FolderQueries.getRootContent(workspaceId));
  await context.data.file.deleteManyByQuery(FolderQueries.getRootContent(workspaceId));
...
async function internalDeleteFoldersByWorkspaceId(context: IBaseContext, workspaceId: string) {
  await internalDeleteFolderList(
    ...
    await context.data.folder.getManyByQuery(
      // Root folders
      FolderQueries.getRootContent(workspaceId)
    )
  );
}
