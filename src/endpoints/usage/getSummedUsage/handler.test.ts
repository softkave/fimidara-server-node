...
import getSummedUsage from './handler.js';
...
describe('getSummedUsage', () => {
  test('get all summed records', async () => {
    ...
    // run
    const reqData =
      RequestData.fromExpressRequest<GetSummedUsageEndpointParams>(
        mockExpressRequestWithAgentToken(userToken),
        {workspaceId: workspace.resourceId}
      );
    const result = await getSummedUsage(reqData);
  ...
  test('get query summed records', async () => {
    ...
    // run
    // with preset category
    let reqData = RequestData.fromExpressRequest<GetSummedUsageEndpointParams>(
      mockExpressRequestWithAgentToken(userToken),
      {
        workspaceId: workspace.resourceId,
    ...
    const result01 = await getSummedUsage(reqData);

    // with preset fulfillment status
    reqData = RequestData.fromExpressRequest<GetSummedUsageEndpointParams>(
      mockExpressRequestWithAgentToken(userToken),
      {
        workspaceId: workspace.resourceId,
    ...
    const result02 = await getSummedUsage(reqData);
    ...
    const result03 = await getSummedUsage(reqData);
    ...
    const result04 = await getSummedUsage(reqData);
  ...
  test('pagination', async () => {
    ...
    let result = await getSummedUsage(reqData);
    assertEndpointResultOk(result);
    ...
    result = await getSummedUsage(reqData);
    assertEndpointResultOk(result);
    expect(result.page).toBe(page);
    expect(result.records).toHaveLength(
      calculatePageSize(count, pageSize, page)
