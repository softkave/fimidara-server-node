...
import sysDeleteFile from './deleteFile/handler.js';
import {
...
import sysReadFile from './readFile/handler.js';
import {
...
async function handleReadFileResponse(
  res: Response,
  result: Awaited<ReturnType<SysReadFileEndpoint>>,
  req: Request,
  input: SysReadFileEndpointParams
) {
  if (input.download) {
    const filename =
      result.name +
...
function extractReadFileParamsFromReq(req: Request): SysReadFileEndpointParams {
