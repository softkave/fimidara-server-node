...
import sysReadFile from './handler.js';
...
describe('readFile', () => {
  ...
  test.each(kTestSessionAgentTypes)(
    ...
    async agentType => {
      ...
      try {
        const reqData =
          RequestData.fromExpressRequest<SysReadFileEndpointParams>(
            mockExpressRequestForPublicAgent(),
            {filepath: stringifyFilenamepath(file, workspace.rootname)}
          );
        await sysReadFile(reqData);
      } catch (error) {
