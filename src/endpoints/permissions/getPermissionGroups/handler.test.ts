...
import getPermissionGroups from './handler.js';
import {GetPermissionGroupsEndpointParams} from './types.js';
...
describe('getPermissionGroups', () => {
  test("workspace's permissionGroups returned", async () => {
    ...
    const reqData =
      RequestData.fromExpressRequest<GetPermissionGroupsEndpointParams>(
        mockExpressRequestWithAgentToken(userToken),
        {workspaceId: workspace.resourceId}
      );
    const result = await getPermissionGroups(reqData);
  ...
  test('pagination', async () => {
    ...
    let result = await getPermissionGroups(reqData);
    assertEndpointResultOk(result);
    ...
    page = 1;
    reqData = RequestData.fromExpressRequest<GetPermissionGroupsEndpointParams>(
      mockExpressRequestWithAgentToken(userToken),
      {page, pageSize, workspaceId: workspace.resourceId}
    );
    result = await getPermissionGroups(reqData);
    assertEndpointResultOk(result);
    expect(result.page).toBe(page);
    expect(result.permissionGroups).toHaveLength(
      calculatePageSize(count, pageSize, page)
