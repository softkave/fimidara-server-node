...
import getCollaborationRequestsEndpoint from './handler.js';
...
describe('getWorkspaceRequests', () => {
  test('workspace collaboration requests returned', async () => {
    ...
    const reqData =
      RequestData.fromExpressRequest<GetCollaborationRequestsEndpointParams>(
        mockExpressRequestWithAgentToken(userToken),
        {workspaceId: workspace.resourceId}
      );
    const result = await getCollaborationRequestsEndpoint(reqData);
  ...
  test('pagination', async () => {
    ...
    let result = await getCollaborationRequestsEndpoint(reqData);
    assertEndpointResultOk(result);
    ...
    result = await getCollaborationRequestsEndpoint(reqData);
    assertEndpointResultOk(result);
    expect(result.page).toBe(page);
    expect(result.requests).toHaveLength(
      calculatePageSize(count, pageSize, page)
