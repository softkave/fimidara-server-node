      ...
      return new Promise<ISendRequestEmailResult>(resolve => {
        context
          .sendRequestEmail(
            context,
            request,
            !!existingUsersMap[request.recipientEmail.toLowerCase()],
            organizationName
          )
          .then(() => {
            resolve({
...
const sendRequests: SendRequestsEndpoint = async (context, instData) => {
  ...
  let errors = existingUsers
    .filter(existingUser => {
      return (
        existingUser.organizations.findIndex(
          next => next.organizationId === organization.organizationId
        ) !== -1
      );
    })
    .map(existingUser => {
  ...
  fireAndForgetPromise(
    sendEmails(context, savedRequests, existingUsers, organization.name)
  );
